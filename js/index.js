function circleClick(event) {
  ...
  const circleTop = rect.top + Math.floor(rect.height / 2);
  const circleLeft = rect.left + Math.floor(rect.width / 2);
  ...
  const squareTop = classList.find(
    (clazz) => clazz === "top" || clazz === "bottom"
  )
    ? calculateNew(classList.includes("bottom"), parentRect.top, 200)
    : parentRect.top;
  const squareLeft = classList.find(
    (clazz) => clazz === "left" || clazz === "right"
  )
